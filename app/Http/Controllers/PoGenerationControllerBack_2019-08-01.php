...
class PoGenerationController extends Controller
{
    ...
    public function poGenerateView($id=null){
        ...
        return View::make('po-generation.create')->with(compact('demandId','selectedSupplier','demandInfo','qyeryResutl','poNumberCreate','tenderNumber','tenderId'));
    ...
    public function postPoGenerate(Request $request){
        ...
        $data['selectedSupplier']  = DemandToCollectionQuotation::where('demand_id','=',$data['demand_id'])->where('tender_id','=',$tenderId)->where('winner','=',1)->first();
        ...
        \DB::table('item_to_demand')
            ->where('demand_id', $data['demand_id'])
        ...
        // =====================================================
            ...
            //======================================================
            $demand_to_coll_supp = DemandToCollectionQuotation::where('demand_id','=',$demand_id)->where('tender_id','=',$tenderId)->where('winner','=',1)->first();
                        ...
                        if(!empty($squeryResult)){
                            ...
                            foreach ($squeryResult as $val) {
                                ...
                                if(count($checkAlreadyExits)<1){
                                    ...
                                    if ($v->fails()) {
                                        return redirect('po-generate-view/'.$demand_id.'&'.$tenderId)->withErrors($v->errors())->withInput();
                                    }
