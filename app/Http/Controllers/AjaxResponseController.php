...
// 
    ...
    public function getSupplierbyMobileNumber(Request $request){

        $suppliersName  = Supplier::select('id','company_name','mobile_number');
        $suppliersName->where('mobile_number', 'LIKE', "%{$request['query']}%");
        $suppliersName->where('enlistment_status','pending');
        $suppliersName = $suppliersName->limit(20)->get();
        ...
        foreach($suppliersName as $row)
        {
            $output .= '<li class="searchMobileNumber" value="'.$row->id.'"><a href="javascript:void(0)">'.$row->mobile_number.'</a></li>';
        }
        $output .= '</ul>';
        echo $output;
    ...
    public function getSupplierbyMobileNumberBarcode(Request $request){
        $this->tableAlies = \Session::get('zoneAlise');

        $suppliersName = SupplierMultiInfo::leftJoin($this->tableAlies.'_suppliers', $this->tableAlies.'_supplier_multi_info.supplier_id', '=', $this->tableAlies.'_suppliers.id')
            ->select($this->tableAlies.'_suppliers.id', $this->tableAlies.'_suppliers.company_name', $this->tableAlies.'_suppliers.mobile_number', $this->tableAlies.'_suppliers.company_regi_number_nsd', $this->tableAlies.'_suppliers.trade_license_address',$this->tableAlies.'_supplier_multi_info.barcode_number',$this->tableAlies.'_supplier_multi_info.name')
            ->OrWhere( $this->tableAlies.'_supplier_multi_info.barcode_number', 'LIKE', "%{$request['query']}%")
            ->limit(20)
            ->get();
        ...
        foreach($suppliersName as $row)
        {
            $output .= '<li class="searchMobileNumber" value="'.$row->id.'"><a href="javascript:void(0)">'.$row->barcode_number.'</a></li>';
        }
        $output .= '</ul>';
    ...
    public function getSupplierbyBarcode(Request $request){
        ...
        $this->tableAlies = \Session::get('zoneAlise');
        $suppliers = SupplierMultiInfo::leftJoin($this->tableAlies.'_suppliers', $this->tableAlies.'_supplier_multi_info.supplier_id', '=', $this->tableAlies.'_suppliers.id')
            ->select($this->tableAlies.'_suppliers.id', $this->tableAlies.'_suppliers.company_name', $this->tableAlies.'_suppliers.mobile_number', $this->tableAlies.'_suppliers.company_regi_number_nsd', $this->tableAlies.'_suppliers.trade_license_address',$this->tableAlies.'_supplier_multi_info.barcode_number',$this->tableAlies.'_supplier_multi_info.name')
            ->Orwhere( $this->tableAlies.'_supplier_multi_info.barcode_number',$request['barcode_number']);
    ...
    public function getSupplier(Request $request){
            ...
            $suppliers  = Supplier::where('mobile_number', 'LIKE', "%{$request['mobile_number']}%" );
            $suppliers = $suppliers->first();
