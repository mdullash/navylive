...
// 
    ...
    public function supplierSearchByRegRrBarNo(Request $request){
        ...
        if($request->get('query2'))
        {
            ...
            foreach($suppliersName as $row)
            {
                $output .= '<li class="searchSuppName" value="'.$row->id.'" fullname="'.$row->full_name.'" companyname="'.$row->company_name.'" address="'.$row->trade_license_address.'"><a href="javascript:void(0)">'.$row->company_regi_number_nsd.'</a></li>';
            }
            $output .= '</ul>';
            echo $output;
        }
