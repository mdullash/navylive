...
class EvaluatedTenderController extends Controller
{
    ...
    {   
        ...
        $supplierstn = $this->tableAlies.'_suppliers.id';
        ...
        $queryResult   = \App\Supplier::leftJoin('evaluated_tender','evaluated_tender.supplier_id','=',$this->tableAlies.'_suppliers.id')
            ->leftJoin('evaluation_criteria','evaluation_criteria.id','=','evaluated_tender.evalu_citeria_id')
            ->leftJoin($this->tableAlies.'_tenders',$this->tableAlies.'_tenders.id','=','evaluated_tender.tender_id')
            ->select($this->tableAlies.'_suppliers.id as supplirId',$this->tableAlies.'_suppliers.company_name',
            \DB::raw("(select SUM((evaluated_tender.marks)*evaluation_criteria.weight) FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.evalu_citeria_id = 1 ) as c1, (select count((evaluated_tender.marks)) FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.evalu_citeria_id = 1  && evaluated_tender.marks != 0 ) as c1c, (select evaluated_tender.citeria_comment FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.evalu_citeria_id = 1 && evaluated_tender.id = ( SELECT MAX(evaluated_tender.id) FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.citeria_comment IS NOT NULL && evaluated_tender.evalu_citeria_id = 1 ) ) as c1cm"),
        ...
        if(!empty($sup_id)){
            $queryResult->where($this->tableAlies.'_suppliers.id','=',$sup_id);
        }
        if(!empty($from) || !empty($todate) || !empty($ten_number)){
            $queryResult->whereIn('evaluated_tender.tender_id',$tenderIds);
        }
        if(!empty(\Auth::user()->nsd_bsd)){
            $queryResult->whereRaw("find_in_set('".\Auth::user()->nsd_bsd."', nsd_suppliers.registered_nsd_id)");
        }
        //->where($this->tableAlies.'_suppliers.id','=',371)
        $queryResult->groupBy($this->tableAlies.'_suppliers.id');
        $queryResult = $queryResult->paginate(50);
    ...
    {   
        ...
        $queryResult   = \App\Supplier::leftJoin('evaluated_tender','evaluated_tender.supplier_id','=',$this->tableAlies.'_suppliers.id')
            ->leftJoin('evaluation_criteria','evaluation_criteria.id','=','evaluated_tender.evalu_citeria_id')
            ->leftJoin($this->tableAlies.'_tenders',$this->tableAlies.'_tenders.id','=','evaluated_tender.tender_id')
            ->leftJoin('demand_to_collection_quotation','demand_to_collection_quotation.supplier_name','=',$this->tableAlies.'_suppliers.id') // newly added
            ->select($this->tableAlies.'_suppliers.id as supplirId',$this->tableAlies.'_suppliers.company_name',
            \DB::raw("(select SUM((evaluated_tender.marks)*evaluation_criteria.weight) FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.evalu_citeria_id = 1 ) as c1, (select count((evaluated_tender.marks)) FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.evalu_citeria_id = 1  && evaluated_tender.marks != 0 ) as c1c, (select evaluated_tender.citeria_comment FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.evalu_citeria_id = 1 && evaluated_tender.id = ( SELECT MAX(evaluated_tender.id) FROM evaluated_tender WHERE ".$supplierstn." = evaluated_tender.supplier_id && evaluated_tender.citeria_comment IS NOT NULL && evaluated_tender.evalu_citeria_id = 1) ) as c1cm"),
            ...
            \DB::raw("(select count(DISTINCT (demand_to_collection_quotation.tender_id)) FROM demand_to_collection_quotation WHERE ".$supplierstn." = demand_to_collection_quotation.supplier_name ) as nop")
        );
        if(!empty($sup_id)){
            $queryResult->where($this->tableAlies.'_suppliers.id','=',$sup_id);
        }
