...
class NhqApprovalController extends Controller
{
    ...
    public function headquarteApproval($id){
        ...
        if($mainTenderInfo->tender_nature != 2){
            ...
            return View::make('nhq-approval.cst-draft-view-item')->with(compact('itemList','supplierResult','targetArray','supArray','supTotalAmountArray','demandId','supplierAllreadySelected','supWiComArray','demand','tenderId','tender','suppliersInf','orgInfo'));
        }else{
            ...
            return View::make('nhq-approval.cst-draft-view-lot')->with(compact('itemList','supplierResult','targetArray','supArray','supTotalAmountArray','demandId','supplierAllreadySelected','supWiComArray','demand','tenderId','tender','suppliersInf','supplierResultFir','mainArray','orgInfo'));
    ...
    public function postHeadquarteApproval(Request $request){
        ...
        if($request->tenderNature==2){
            ...
            if ($validator->fails()) {
                return redirect('headquarte-approval/'.$demand_id.'&'.$tenderId)->withErrors($validator);
            } else {
                ...
                if(count($request->cst_draft_sup_id)>0 ){
                    ...
                    \DB::table('demand_supplier_to_coll_qut_to_item')
                        ...
                        $retenderAlreadyExist = \App\Retender::where('tender_id','=',$tenderId)->where('tender_number','=',$tender->tender_number)->where('demand_id','=',$demand_id)->first();
                        if(in_array(2,$request->nhq_app_status)){
                            $demandToTen = \App\DemandToTender::where('demand_id','=',$demand_id)->where('tender_id', $tenderId)->first();
                            ...
                            $createRetender->demand_id      = $demand_id;
