                                    ...
                                    <div class="col-md-12 remove appendRewRow firstRow">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control selectpicker supplier_id" name="supplier_name[{!! $sl !!}][]" id="supplier_id" data-live-search="true" required>
                                                    <option value="">{!! '- Select -' !!}</option>
                                                        @foreach($suppliers as $sup)
                                                            <option value="{!! $sup->id !!}" @if($sup->id==$asgd->supplier_name) selected @endif>{!! $sup->company_name !!}</option>
                                                        @endforeach
                                                </select>
                                            </div>
                                            <input type="hidden" class="form-control search_supplier_id" id="search_supplier_id" name="suppliernametext[{!! $sl !!}][]" value="{!! $asgd->suppliernametext !!}" placeholder="Search supplier..." autocomplete="off" required>
                                        </div>
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control selectpicker item_id" name="item_id[{!! $sl !!}][]" id="item_id" data-live-search="true" >
                                                    <option value="">{!! '- Select -' !!}</option>
                                                        @foreach($items as $itn)
                                                            <option value="{!! $itn->id.'&'.$itn->item_id !!}" attrolditemid="{!! $itn->item_id !!}" @if($itn->id==$selitm->item_id) selected @endif>{!! $itn->item_name !!}</option>
                                                        @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-3 ">
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control unit_price" id="unit_price" name="unit_price[{!! $sl !!}][]" value="{!! $selitm->unit_price !!}" min="0" >
                                            </div>
                                        </div>
                                        ...
                                        @foreach($items as $itn)    
                                            <div class="col-md-12 supplierUnderRows">
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label for="deno">Item Name :<span class="text-danger">*</span></label>
                                                        <select class="form-control selectpicker item_id" name="item_id[0][]" id="item_id" data-live-search="true" >
                                                            <option value="">{!! '- Select -' !!}</option>
                                                                @foreach($items as $itn)
                                                                    <option value="{!! $itn->id.'&'.$itn->item_id !!}" attrolditemid="{!! $itn->item_id !!}" attritemqty="{!! $itn->unit !!}">{!! $itn->item_name !!}</option>
                                                                @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-3 ">
                                                    <div class="form-group">
                                                        <label for="unit_price">Unit Price:<span class="text-danger">*</span></label>
                                                        <input type="number" class="form-control unit_price" id="unit_price" name="unit_price[0][]" min="0" >
                                                    </div>
                                                </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        // ========================================================================================================
            ...
            $(document).on('click','#addNewRow',function(){
                ...
                $.each(items, function (key, value) {
                    $('#supplierUnderRows'+oli).append('<div class="col-md-12 supplierUnderRows"><div class="col-md-3"><div class="form-group"><label for="deno">Item Name :<span class="text-danger">*</span></label><select class="form-control selectpicker item_id append_item_id'+oli+'" name="item_id['+oli+'][]" id="item_id'+i+'" data-live-search="true" ><option value="">- Select -</option></select></div></div><div class="col-md-3"><div class="form-group"><label for="unit_price">Unit Price:<span class="text-danger">*</span></label><input type="number" class="form-control unit_price" id="unit_price" name="unit_price['+oli+'][]" min="0" ></div></div><div class="col-md-3"><div class="form-group"><label for="discount_amount">Discount Amount:</label><input type="number" class="form-control discount_amount" id="discount_amount" name="discount_amount['+oli+'][]" min="0"></div></div><div class="col-md-3"><div class="form-group"><label for="quantity">Quantity:<span class="text-danger">*</span></label><input type="number" class="form-control quantity" id="quantity" name="quantity['+oli+'][]" min="0" readonly="" required></div></div><div class="col-md-3 hidden"><div class="form-group"><label for="total_price">Price:<span class="text-danger"></span></label><input type="number" class="form-control total_price" id="total_price" name="total_price['+oli+'][]" min="0" readonly="" required></div></div><div class="col-md-2 hidden"><div class="form-group"><label for="last_unti_price">Last Purchase Price:<span class="text-danger">*</span></label><input type="number" class="form-control last_unti_price" id="last_unti_price" name="last_unti_price['+oli+'][]" min="0" readonly="" required></div></div></div>');

                });

                $.each(items, function (key, value) {
                    $('.append_item_id'+oli).append("<option value='"+ value.id+'&'+value.item_id+"' attrolditemid='"+ value.item_id+"' attritemqty='"+ value.unit+"'>"+value.item_name+"</option>");
        ...
        $(document).on('keyup','.search_supplier_id',function() { 
            var query = $(this).val();
            var demandId = "<?php echo $demand_id; ?>";
        ...
        // =================================================
         $(document).on('change','.item_id',function(){
            ...
            $.ajax({
                url: '{{url('create-collection-quotation-item-info') }}',
