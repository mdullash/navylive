                        ...
                        <div class="panel-body">
                            ...
                            <div class="row">
                                ...
                                <ul class="nav nav-tabs">
                                    <?php if(!empty(Session::get('acl')[34][35]) && $demandDetailPageFromRoute=='v44-voucher-view-acc'){ ?>
                                        <li @if($demandDetailPageFromRoute=='v44-voucher-view-acc' && $segTwo==1)class="active" @endif><a href="{{URL::to('v44-voucher-view-acc/1')}}"> Pending</a></li>
                                    <?php } ?>
                                    <?php if(!empty(Session::get('acl')[34][36]) && $demandDetailPageFromRoute=='v44-voucher-view-acc'){ ?>
                                    <li @if($demandDetailPageFromRoute=='v44-voucher-view-acc' && $segTwo==2)class="active" @endif><a href="{{URL::to('v44-voucher-view-acc/2')}}"> Waiting for approve</a></li>
                                    <?php } ?>
                                    <li @if($demandDetailPageFromRoute=='v44-voucher-view-acc' && $segTwo==3)class="active" @endif><a href="{{URL::to('v44-voucher-view-acc/3')}}"> Approved</a></li>
                                </ul>
                                ...
                                @if (!$demands->isEmpty())
                                    ...
                                    @foreach($demands as $sc)
                                        <tr>
                                            ...
                                            <td>
                                                @if($segTwo==1)
                                                    <a class="btn btn-success btn-xs" href="{{ URL::to('v44voucher-view/'.$sc->id) }}" title="D44B view">
                                                        <i class="icon-check"></i>
                                                    </a>
                                                @endif
                                                @if($segTwo==2)
                                                    <a class="btn btn-success btn-xs" href="{{ URL::to('v44voucher-pdf-view/'.$sc->id.'&1') }}" title="Check PO" target="_blank">
                                                            <i class="icon-eye-open"> </i>
                                                    </a>

                                                    <a class="btn btn-primary btn-xs showModal" href="javascript:void(0)" title="Approve Demand" attr-demandid-updateflds="{!! $sc->id !!}">
                                                        <i class="icon-check"> </i>
                                                    </a>
                                                    
                                                @endif
                                                @if($segTwo==3)
                                                    <a class="btn btn-primary btn-xs" href="{{ URL::to('v44voucher-pdf-view/'.$sc->id.'&2') }}" title="Check PO" target="_blank">
