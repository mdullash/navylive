...
<table style="width: 100%;font-size: 14px;">
    ...
    <tr>
        <td>
            {{!empty($tenderFirst->tender_number) ? $tenderFirst->tender_number : ''}}{{!empty($nilReturn->nil_number) ? '-'.$nilReturn->nil_number : ''}}
        </td>
        <td></td>
...
<p>a. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Tender Reference No {{ !empty($tenderFirst->nhq_ltr_no) ? $tenderFirst->nhq_ltr_no : '' }} Date {{ !empty($tenderFirst->reference_date) ? date('d F Y',strtotime($tenderFirst->reference_date)) : '' }}.</p>
...
//	tender First
     ...
     if ($tenderFirst->tender_type == 1)
     	{
	        $tenderTypeFirst = 'LTM- Limited Tender Method';
        }
        else if ($tenderFirst->tender_type == 2)
        {
	        $tenderTypeFirst =  'OTM- Open Tender Method';
        }
        ...
        {
	        $tenderTypeFirst =  'DPM- Direct Purchase Method';
        }
        else
        {
    ...
    //        tender latest
        ...
        {
	        $tenderTypeLatest = 'LTM- Limited Tender Method';
        }
        else if ($tendertLatest->tender_type == 2)
        {
	        $tenderTypeLatest =  'OTM- Open Tender Method';
        }
        ...
        {
	        $tenderTypeLatest =  'DPM- Direct Purchase Method';
        }
        else
        {
    ...
    ?>
    In light of Ref ‘a’ Open {{$tenderTypeFirst}} Tender has been floated for the above mentioned item and open on {{date('d F Y',strtotime($tenderFirst->tender_opening_date))}}.
    {{ ($qutationFirstTenderCount > 0) ? ucwords(\functions\OwnLibrary::numberTowords1($qutationFirstTenderCount)) : "No"  }} {{$qutFirst}} have been found. Received quotation from
    {{$supplierNameFirst}} has been cancelled due to not fulfilling tender requirements.
    Later Open {{$tenderTypeLatest}} Tender again floated and open on {{$retenderDates}} consecutively.
    {{ ($qutationLatestTenderCount > 0) ? ucwords(\functions\OwnLibrary::numberTowords1($qutationLatestTenderCount)) : "No" }} {{$qutLatest}} have been found. Received quotation from {{$supplierNameLatest}}
    has been cancelled due to not fulfilling tender requirements.
</p>

<p style="text-align: justify;line-height: 170%;margin-top: 30px;">2. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    Hence, Nil return is forwarded for necessary action.
</p>
<br /><br /><br /><br /><br />
