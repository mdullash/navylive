...
<p>a. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{!! (!empty($tenderInfo->nhq_ltr_no)) ? $tenderInfo->nhq_ltr_no : '' !!}  Date {!! (!empty($tenderInfo->nhq_ltr_date)) ? date('d F Y',strtotime($tenderInfo->nhq_ltr_date)) : '' !!}<br />
    b. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NSSD Purchase Order No. {{!empty($podata->po_number) ? $podata->po_number : ''}} Date {{!empty($podata->top_date) ? date('d F Y',strtotime($podata->top_date)) : ''}}.<br />
    c. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NSSD Ltr No {{ !empty($billForwarding->nssd_ltr_no) ? $billForwarding->nssd_ltr_no : '' }}
    Date {{ !empty($billForwarding->nssd_ltr_date) ? date('d F Y',strtotime($billForwarding->nssd_ltr_date)) : '' }}.<br />
    d. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ !empty($demandToColQut->suppliernametext) ? $demandToColQut->suppliernametext : '' }} Bill no {{ !empty($billForwarding->bill_number) ? $billForwarding->bill_number : '' }}
...
<p style="text-align: justify;line-height: 170%;margin-top: 20px;">
    1. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In view of approval of Ref 'a' and financial sanction, items have been delivered successfully by {{ !empty($demandToColQut->suppliernametext) ? ucwords($demandToColQut->suppliernametext) : '' }}.
    As such necessary papers are been sent for bill payment of  Taka {{ \functions\OwnLibrary::numberformat($demandToColQut->total) }}
...
<p style="text-align: justify;line-height: 170%;margin-top: 20px;">
    3. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Application for delivery time extension already sent by ref 'c'. As such, deduction of LD is not recommended.
</p>
