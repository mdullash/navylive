...
<p>a. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{!! (!empty($tenderInfo->nhq_ltr_no)) ? $tenderInfo->nhq_ltr_no : '' !!}  Date {!! (!empty($tenderInfo->nhq_ltr_date)) ? date('d F Y',strtotime($tenderInfo->nhq_ltr_date)) : '' !!}<br />
    b. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NSSD Purchase Oder No. {{!empty($podata->po_number) ? $podata->po_number : ''}} Date {{!empty($podata->top_date) ? date('d F Y',strtotime($podata->top_date)) : ''}}.<br />
    ...
    Date {{ !empty($billForwarding->bill_date) ? date('d F Y',strtotime($billForwarding->bill_date)) : '' }}. <br />
    d. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ !empty($demandToColQut->suppliernametext) ? $demandToColQut->suppliernametext : '' }} Application No {{ !empty($billForwarding->time_ext_app_no) ? $billForwarding->time_ext_app_no : '' }}
    Date {{ !empty($billForwarding->time_ext_app_date) ? date('d F Y',strtotime($billForwarding->time_ext_app_date)) : '' }}.
</p>



<p style="text-align: justify;line-height: 170%;margin-top: 20px;">
    1. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In view of approval of Ref 'a' items have been delivered successfully by {{ !empty($demandToColQut->suppliernametext) ? $demandToColQut->suppliernametext : '' }}.
    As such necessary document sent for bill payment of Taka {{ \functions\OwnLibrary::numberformat($demandToColQut->total) }}
...
<p style="text-align: justify;line-height: 170%;margin-top: 20px;">3. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;The supplier has applied for the delivery time extension up to {{ !empty($billForwarding->time_ext_up_to) ? date('d F Y',strtotime($billForwarding->time_ext_up_to)) : "" }} in ref 'c'. This application is also attached for necessary action please.
</p>
