...
<!-- Add new row section ==========================================================     -->
                    ...
                    <div class="row" style="text-align: center;" id="theaderRow">
                        ...
                        <div class="col-md-2"><b>Currency</b></div>
                        <div class="col-md-1"><b>Conversion</b></div>
                        <div class="col-md-2"><b>Unit Price</b></div>
                        <div class="col-md-1"><b>Qty.</b></div>
                        <div class="col-md-1"><b>Dis. P/U</b></div>
                        <div class="col-md-2"><b>Total Tk</b></div>
                    ...
                    @if(count($itemAlreadyAssign)>0)
                        ...
                        <div class="row remove firstRow" id="firstRow">
                            ...
                            <div class="col-md-2">
                                <select class="form-control selectpicker currency_name" name="currency_name[]" id="currency_name"  data-live-search="true">
                                    <option value="">{!! '- Select -' !!}</option>
                                    @foreach($currencies_names as $csn)
                                        <option value="{!! $csn->id !!}" conversionrate="{!! $csn->conversion !!}" @if(!empty($iaa->currency_name) && $csn->id==$iaa->currency_name) {!! 'selected' !!} @else @if($default_currency) @if($default_currency->id==$csn->id) {!! 'selected' !!} @endif @endif @endif>{!! $csn->currency_name !!}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-1">
                                <?php
                                    $conv = !empty($iaa->conversion) ? $iaa->conversion : $default_currency->conversion;
                                ?>
                                {!!  Form::text('conversion[]', $conv, array('class' => 'form-control conversion', 'id' => 'conversion')) !!}
                            </div>
                            <div class="col-md-2">
...
<script type="text/javascript">
    ...
    $(document).on("click","#addNewRow",function(){
        ...
        $("#firstRow").after( '<div class="row remove firstRow" style="padding-top : 10px;"><div class="col-md-2"><input type="text" class="form-control search_item_name" id="search_item_name" name="search_item_name" autocomplete="off" placeholder="Search Item ..." /> <input type="hidden" class="form-control item_id" name="item_id[]" id="item_id" /> <div class="form-group col-xs-12 col-sm-12 col-md-6 search_itmem_name_div" id="search_itmem_name_div" style="display: none; display: block; position: absolute; left: 16px;"></div></div><div class="col-md-2"><select class="form-control selectpicker currency_name" name="currency_name[]" id="currency_name'+i+'" data-live-search="true"><option value="">- Select -</option></select></div><div class="col-md-1"><input type="text" value="'+conv+'" class="form-control conversion" id="conversion" name="conversion[]" /></div><div class="col-md-2"><input type="text" class="form-control unit_price" name="unit_price[]" /></div><div class="col-md-1"> <input type="text" class="form-control quantity" name="quantity[]" /></div><div class="col-md-1"><input type="text" class="form-control discount_price" name="discount_price[]" /></div><div class="col-md-2"><input type="text" class="form-control total" name="total[]" readonly ="readonly"/></div><div class="col-md-1"><button class="btn btn-danger removeRow" id="" type="button" data-placement="top" data-rel="tooltip" data-original-title="Remove"><i class="fa fa-trash"></i></button></div></div>' );

        var curriencies = <?php echo json_encode($currencies_names); ?>;
        var defaultSelected = <?php echo $default_currency->id; ?>;
        $.each(curriencies, function (key, value) {
            $('#currency_name'+i).append("<option value='"+ value.id+"' conversionrate='"+value.conversion+"' "+(value.id==defaultSelected ? "selected=selected" : '')+" >"+value.currency_name+"</option>");
        });
        $('#currency_name'+i).selectpicker('refresh');
        i++;
    ...
    $(document).on("input",".quantity,.discount_price,.unit_price,.conversion",function(){
        ...
        var conversionRate = $(this).closest("div.remove").find(".conversion").val();

        
        var total = parseFloat(((unitPrice*quantity)*conversionRate)-((quantity*discountPrice))*conversionRate).toFixed(2);
        ...
        $(document).on('click', '.searchItemName', function () {
            ...
            if(curCurrency != ''){
                $(this).closest("div.remove").find(".currency_name").val(curCurrency).change();
                $(this).closest("div.remove").find(".conversion").val(curConversion);
            }else{
                $(this).closest("div.remove").find(".conversion").val(1);
            }
        ...
        $(document).on('change','.currency_name',function(){
            ...
            $(this).closest("div.remove").find(".conversion").val(value);
            $(this).closest("div.remove").find(".conversion").trigger('input');
        });
