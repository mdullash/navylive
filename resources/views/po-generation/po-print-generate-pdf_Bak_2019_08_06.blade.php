    ...
    @foreach($demandToCollQut as $dtcq)
        <table style="width: 100%;">
            <!-- <tbody style="width: 215.9mm;"> -->
                ...
                <tr>
                    ...
                    <td style=""></td>
                    <td ></td>
                    <td style="font-size: 14px; text-align: right; width: 65mm;">{!! (!empty($podataInfo->top_date)) ? date('d',strtotime($podataInfo->top_date)) : '' !!}</td>
            ...
            <table>
                <tr>
                    <td colspan="3" style=" font-size: 14px; font-weight: bold;"><span style="border-bottom: 1px solid black;">LOCAL PURCHASE ORDER-{!! (!empty($tenderInfo->tender_title)) ? $tenderInfo->tender_title : '' !!}-BNS PROPERTY</span></td>
                </tr>
                <tr>
                    <td></td>
                </tr>
                ...
                @if(!empty($demandToLprInfo->head_ofc_apvl_status))
                <tr>
                    <td colspan="3" style=" font-size: 14px;">B .&nbsp;&nbsp;&nbsp;Naval Headquarters Letter no. {!! (!empty($podataInfo->headquarters_letter_no)) ? $podataInfo->headquarters_letter_no : '' !!} Date {!! (!empty($tenderInfo->valid_date_from)) ? date('d F Y',strtotime($tenderInfo->valid_date_from)) : '' !!}.</td>
                </tr>
                @endif
            </table>
            <br>
            <table>
                <tr>
                    ...
                    ?>
                    <td colspan="3" style=" font-size: 15px;text-align: justify;"><span>1 .</span>&nbsp;&nbsp;In light of approval <span style="">vide</span> ref 'b', above mentioned item <span style=" ">@if(array_sum(array_map("count", $selectedSupItemInfo)) <=1) is @else are @endif</span> accepted by Bangladesh Navy as per demand @if(!empty($tenderInfo->specification) || !empty($tenderInfo->specification_doc)) and specification @endif. You are hereby awarded the work order of Taka {!! $totalAmountAfterDiscount !!} ({!! OwnLibrary::numberTowords($totalAmountAfterDiscount) !!}) <span style="">{!! (!empty($podataInfo->import_duties)) ? $podataInfo->import_duties : '' !!} import duties.</span> In this context you are requested to supply the item to <span style="">{!! (!empty($podataInfo->supply_to)) ? $podataInfo->supply_to : '' !!}</span> by complying the following conditions mentioned below:</td>
                </tr>
                <tr style="height: 2mm;"></tr>
            <!-- </tbody> -->
        </table>
        ...
        <table  style="width: 100%;">
            <!-- <tbody  style="width: 215.9mm;"> -->
                ...
                <tr>
                    <td colspan="6" style=" font-size: 14px;">1.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Contract Specification -01(One) page. </td>
                </tr>
                <tr>
                    <td style="height: 2mm" colspan="6"></td>
                </tr>
...
<htmlpagefooter name="page-footer">
        <table style="vertical-align: bottom; font-family: serif; color: #000000;" width="100%">
            ...
            <tr>
                ...
                <td style=" font-style: italic; font-size: 14px;" align="right" width="31%;">{!! date('d-m-Y') !!}</span></td>
            </tr>
            </tbody>
        </table>
    </htmlpagefooter>
