        ...
        <div class="row">
            <div class="col-sm-6 col-md-12">
                <div class="hpanel">
                    ...
                    <div class="panel-body">
                        {{ Form::model($editId, array('route' => array('contact.update', $editId->id), 'method' => 'PUT', 'files'=> true, 'class' => 'form form-horizontal validate-form contact', 'id' => 'contact')) }}
                        ...
                        <div class="form-group"><label class="control-label col-md-3 no-padding-right" for="stall_id">Zone :<span class="text-danger">*</span></label>
                            <div class="col-md-6">
                                <select class="form-control selectpicker" name="zones" id="zones"  data-live-search="true" required>
                                    ...
                                    @foreach($zones as $zn)
                                        <option value="{!! $zn->id !!}" @if($editId->zones==$zn->id) {!! 'selected' !!} @endif>{!! $zn->name !!}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <select class="form-control selectpicker" name="nsd_bsd" id="nsd_bsd"  data-live-search="true">
                                    @foreach($selectedNsdBsd as $snsdbsd)
                                        <option value="{!! $snsdbsd->id !!}"  @if($snsdbsd->id == $editId->nsd_bsd) {!! 'selected' !!} @endif >{!! $snsdbsd->name !!}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
