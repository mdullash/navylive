...
<head>
    <style>
       ...
       .bg-red{
        ...
        @page :first {
            /*header: page-header;*/
            footer: page-footer-1st-page;
            margin-bottom: 0;
        }

        @page {
            /*header: page-header;*/
            footer: page-footer-other-page;
             margin-bottom: 200px;
        }
...
<body>
<htmlpagefooter name="page-footer-1st-page">
...
<htmlpagefooter name="page-footer-other-page">
    @if(!empty($appUserInfo))
                <div style="margin-bottom: 30px;">
                    <div style="margin: auto; float: right; margin-right: 15px; width: 25%;">
                        ...
                        <p style="margin: 0px;">
                            {!! 
                                !empty($demandToTenderInfo->float_tender_app_rank) ?
                                $demandToTenderInfo->float_tender_app_rank : $appUserInfo->rank 
                            !!}
                        </p>
   ...
   <h2 class="underline text-center">Naval Stores Sub Depot Dhaka<br /> <span class="bg-red"> 
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan="2"><Span class="bg-red">@if(!empty($tenderInfoForPdf->tender_opening_date)) {!! date('d F Y', strtotime($tenderInfoForPdf->tender_opening_date)). ' & Time 1145 hrs'  !!} @else {!! 'NA' !!} @endif</Span></td>
            </tr>
            <tr>
                <td>18.</td>
    ...
    @if($tenderInfoForPdf->is_enclosure == 1)
        ...
        if($tenderInfoForPdf->tender_nature == 1){
        	$itemCountResult = count($itemsInfoDesc);
        }else{
	        $itemCountResult =  array_sum(array_map("count", $lotItemArray));
        }
        ?>
        ...
        <table>
            ...
            <tbody>
                <tr>
                        <td>1</td>
                        <td>
                            @if(!empty($tenderInfoForPdf->tender_title))
                                {{$tenderInfoForPdf->tender_title}}<br />
                            @endif
                            <span style="font-size: 14px;font-weight: bold;">(</span> {{$itemCountResult}} Line Item As Per Attach List <span style="font-size: 14px;font-weight: bold;">)</span>
                        </td>
                        <td>
                            @if(!empty($tenderInfoForPdf->location))
                                {{$tenderInfoForPdf->location}}
                                @endif
                        </td>
                        <td> @if(!empty($tenderInfoForPdf->delivery_date)){!! $tenderInfoForPdf->delivery_date !!} @endif</td>
                        <td> @if(!empty($tenderInfoForPdf->remarks)){!! $tenderInfoForPdf->remarks !!} @endif</td>
                </tr>
            </tbody>
        </table>
    @endif
...
@endif
                        ...
                        @if(in_array($tenderInfoForPdf->id, $tenderIdArr))               
